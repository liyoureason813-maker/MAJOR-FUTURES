...
function applyStaticI18n(){
 ...
 document.title=isEn()?"MAJOR FUTURES | AI-era Major & Career Atlas V3.3":"MAJOR FUTURES｜专业与AI职业地图 V3.3";
...
function openMajor(id,matchSection=""){const m=majors.find(x=>x.id===id),b=band(m.risk),color=categoryColors[m.category];document.querySelector("#dialogContent").innerHTML=`<section class="detail-hero" style="--detail:${color}"><div class="detail-meta">${catName(m.category)} · V3.3 · ${t("detailMeta")}</div><div class="detail-title-row"><div><h2>${majorName(m)}</h2><p>${isEn()?m.name:m.en}</p></div></div><div class="detail-status"><span class="impact-badge" style="--accent:${b.color}">${t("overall")}${colon()}${b.label}</span><span>${t("detailStatus")}</span></div><p class="detail-overview">${overview(m)}</p></section>${matchSection}<div class="kill-line"><strong>${t("killLine")}</strong><p>${isEn()?thresholdEn(m):m.threshold}</p></div><div class="detail-body"><div class="zone-stack">${zone(t("high"),t("zoneHighTag"),m.high,"high")}${zone(t("border"),t("zoneBorderTag"),m.border,"edge")}${zone(t("safe"),t("zoneSafeTag"),m.safe,"safe")}</div><section class="detail-section"><h3>${t("eduTitle")}</h3><p class="section-note">${t("eduNote")}</p><div class="education-scroll"><table class="education-table"><thead><tr><th>${t("colZone")}</th><th>${t("colDirection")}</th><th>${t("colJobs")}</th><th>${t("colEntry")}</th><th>${t("colAdvanced")}</th></tr></thead><tbody>${educationRows(m)}</tbody></table></div></section><section class="detail-section advice-section"><h3>${t("adviceTitle")}</h3><ul class="advice-list">${advice(m).map(x=>`<li>${x}</li>`).join("")}</ul></section></div>`;majorDialog.style.setProperty("--detail",color);majorDialog.showModal();}
